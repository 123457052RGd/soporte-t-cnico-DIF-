...
# Forzar salida en UTF-8 (evita errores de codificación en Windows)
...
PROBLEMAS_COMUNES = {
    'Internet/Red': {
        ...
        'consejo': 'Verifique que el cable de red esté conectado correctamente y reinicie su equipo. Si el problema persiste, envíe el ticket.'
    },
    'Impresora': {
        'opciones': ['No imprime', 'No hay tinta/tóner', 'Atasco de papel', 'Otra'],
        'consejo': 'Verifique si la impresora tiene papel o si el indicador de tinta/tóner está encendido. Si no es un problema físico, envíe el ticket.'
    },
    'Computadora': {
        'opciones': ['Equipo lento', 'Falla al iniciar (pantalla negra)', 'Programa no funciona', 'Otra'],
        'consejo': 'Intente cerrar programas que no use y libere espacio. Si no enciende, envíe el ticket.'
    }
}
    ...
    def wrapper(func):
        ...
        def decorated_view(*args, **kwargs):
            ...
            if role and session.get('role') != role:
                flash(f'Acceso denegado. Rol requerido: {role}.', 'danger')
...
def reportar_incidente():
    ...
    if request.method == 'POST':
        ...
        descripcion = ""  # No se usa descripción adicional

        if not asunto:
            flash("Error: Debe seleccionar y describir el problema.", 'danger')
            return redirect(url_for('reportar_incidente'))

        ticket_id = ticket_dao.crear_ticket(id_cliente, id_activo, asunto, descripcion)
...
def admin_dashboard():
    ...
    tecnicos = [{'id_tecnico': 1, 'nombre': 'Ana Gomez'}]  # ejemplo
...
def cerrar_ticket(id_ticket):
    ...
    if request.method == 'POST':
        ...
        resolucion_dao.documentar_y_cerrar_ticket(
            id_ticket, diagnostico, solucion_aplicada, tiempo_empleado
        )
...
def historial_activo(id_activo):
    ...
    if not activo_info:
        flash("Error: El activo no se encontró en la base de datos.", 'danger')
        return redirect(url_for('admin_dashboard'))

    historial = activo_dao.obtener_historial_por_activo(id_activo)
    if not historial:
        flash(f"No hay historial de soporte para el activo con N/S: {activo_info[0]['serial_number']}.", 'info')

    return render_template(
        'admin_historial_activo.html',
        historial=historial,
        activo_id=id_activo,
